...
Major feature release: smart HTML crawling, form fuzzing, URL parameter injection, blind SQL injection, and CSRF detection.

### New features

**Smart same-domain crawler** — The discovery engine now performs a depth-limited BFS over HTML pages (default depth 4, max 80 pages). It follows `<a href>` links but **strictly restricts traversal to the target domain** — off-domain links are ignored, preventing infinite loops. Static assets (`.png`, `.css`, `.woff`, etc.) are skipped automatically.

**HTML form extraction** — Every `<form>` element on crawled pages is parsed: action URL, HTTP method, and all `<input>` / `<textarea>` / `<select>` fields with their types and names are extracted into `FormSpec` objects.

**Form fuzzer (`FormFuzzer`)** — Each injectable form input is fuzzed with:
- Error-based SQL injection (14 payloads)
- Time-based blind SQLi (MySQL `SLEEP`, MSSQL `WAITFOR DELAY`, PostgreSQL `pg_sleep`)
- Boolean-based blind SQLi (response-size differential analysis)
- XSS reflection (10 payloads, checks for un-encoded reflection)
- Path traversal / LFI (file/path/image-like input names only)
- OS command injection

**URL parameter injector (`URLParamInjector`)** — Injects SQLi / blind SQLi / XSS / LFI / IDOR payloads directly into GET query parameters. Works from:
- Query strings discovered in crawled URLs
- Common parameter wordlist (40+ names: `id`, `q`, `cat`, `file`, `page`, …)
- Parameters extracted from OpenAPI spec endpoints

**CSRF detection** — POST forms without a recognised CSRF token field (`csrf_token`, `_token`, `csrfmiddlewaretoken`, `authenticity_token`, etc.) are flagged as `FindingType.CSRF` / `Severity.MEDIUM`. Only meaningful forms (login, register, transfer, delete, etc.) are reported; static/search forms are skipped.

**Blind SQL injection (time-based and boolean-based)** — Both the form fuzzer and the URL injector independently test for blind injection where no error message is visible.

**New CLI flags:**
- `--no-form-fuzz` — disable form fuzzing
- `--no-url-inject` — disable URL param injection
- `--crawl-depth N` — BFS depth (default 4)
- `--crawl-pages N` — max pages to BFS-crawl (default 80)

### Bug fixes

- `FindingType.CSRF` added to models and SARIF OWASP tag map (`A01:2021`).
- Discovery result now stored on runner instance for downstream modules to access crawled forms and URLs without re-crawling.
