...
class NegativeValueAcceptedRule(AnomalyRule):
    ...
    def check(self, req, resp, context):
        ...
        if isinstance(body, dict):
            for key in ("quantity", "amount", "count", "price", "total"):
...
class IDORRule(AnomalyRule):
    def __init__(self):
        super().__init__(
            name="Potential IDOR",
            description="Request with ID=1 returned 200; may expose another user's data.",
    ...
    def check(self, req, resp, context):
        if resp.status_code != 200:
            return False
        # Heuristic: URL contains /1 or ?id=1 and returned data
        import re
