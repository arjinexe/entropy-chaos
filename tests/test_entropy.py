...
def test_sarif_output_structure():
    ...
    try:
        ...
        assert_eq(runs[0]["tool"]["driver"]["version"], "0.4.4")
...
def test_cli_version_flag():
    ...
    assert_true(
        "0.4.4" in result.stdout or "0.4.4" in result.stderr,
        f"Version output should contain 0.4.4. stdout={result.stdout!r} stderr={result.stderr!r}"
    )
...
def test_shell_non_tty_reads_stdin():
    ...
    import subprocess, sys
    script = (
        "import sys; sys.path.insert(0, '/home/claude/entropy-v0.4.0')\n"
...
# ============================================================
# P. NEW DETECTION RULES (v0.3.4)
# ============================================================

@test
def test_sql_injection_rule_mysql_error():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import SQLInjectionRule
    rule = SQLInjectionRule()
    req  = HTTPRequest("GET", "http://test/search", params={"q": "' OR '1'='1"})
    resp = HTTPResponse(500, body="You have an error in your SQL syntax near ''' OR '1'='1'")
    assert_true(rule.check(req, resp, {}))


@test
def test_sql_injection_rule_clean_response():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import SQLInjectionRule
    rule = SQLInjectionRule()
    req  = HTTPRequest("GET", "http://test/search", params={"q": "hello"})
    resp = HTTPResponse(200, body={"results": []})
    assert_true(not rule.check(req, resp, {}))


@test
def test_xss_reflection_rule():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import XSSReflectionRule
    rule = XSSReflectionRule()
    req  = HTTPRequest("POST", "http://test/comment", body={"text": "<script>alert(1)</script>"})
    resp = HTTPResponse(200, body={"output": "Your comment: <script>alert(1)</script>"})
    assert_true(rule.check(req, resp, {}))


@test
def test_xss_reflection_rule_encoded_safe():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import XSSReflectionRule
    rule = XSSReflectionRule()
    req  = HTTPRequest("POST", "http://test/comment", body={"text": "<script>alert(1)</script>"})
    # Properly encoded — not a finding
    resp = HTTPResponse(200, body={"output": "Your comment: &lt;script&gt;alert(1)&lt;/script&gt;"})
    assert_true(not rule.check(req, resp, {}))
...
def test_path_traversal_rule():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import PathTraversalRule
    rule = PathTraversalRule()
    req  = HTTPRequest("GET", "http://test/download", params={"file": "../../../../etc/passwd"})
    resp = HTTPResponse(200, body="root:x:0:0:root:/root:/bin/bash\ndaemon:x:1:1:")
    assert_true(rule.check(req, resp, {}))


@test
def test_path_traversal_rule_no_trigger():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import PathTraversalRule
    rule = PathTraversalRule()
    req  = HTTPRequest("GET", "http://test/file", params={"name": "report.pdf"})
    resp = HTTPResponse(200, body={"content": "binary data"})
    assert_true(not rule.check(req, resp, {}))


@test
def test_information_disclosure_rule_php_error():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import InformationDisclosureRule
    rule = InformationDisclosureRule()
    req  = HTTPRequest("GET", "http://test/")
    resp = HTTPResponse(500, body="PHP Fatal Error: Call to undefined function mysql_connect()")
    assert_true(rule.check(req, resp, {}))


@test
def test_information_disclosure_rule_stack_trace():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import InformationDisclosureRule
    rule = InformationDisclosureRule()
    req  = HTTPRequest("GET", "http://test/api")
    resp = HTTPResponse(500, body="Traceback (most recent call last):\n  File app.py line 42")
    assert_true(rule.check(req, resp, {}))
...
def test_command_injection_rule():
    from entropy.core.models import HTTPRequest, HTTPResponse
    from entropy.fuzzing.executor import CommandInjectionRule
    rule = CommandInjectionRule()
    req  = HTTPRequest("POST", "http://test/ping", body={"host": "; cat /etc/passwd"})
    resp = HTTPResponse(200, body="uid=0(root) gid=0(root) groups=0(root)")
    assert_true(rule.check(req, resp, {}))


@test
def test_default_rules_includes_new_rules():
    from entropy.fuzzing.executor import DEFAULT_RULES, SQLInjectionRule, XSSReflectionRule, PathTraversalRule
    rule_types = {type(r) for r in DEFAULT_RULES}
    assert_true(SQLInjectionRule in rule_types, "DEFAULT_RULES should include SQLInjectionRule")
    assert_true(XSSReflectionRule in rule_types, "DEFAULT_RULES should include XSSReflectionRule")
    assert_true(PathTraversalRule in rule_types, "DEFAULT_RULES should include PathTraversalRule")
    assert_gt(len(DEFAULT_RULES), 10, "Should have more than 10 default rules")


@test
def test_discovery_php_paths_in_wordlist():
    from entropy.discovery.crawler import COMMON_API_PATHS
    assert_in("/index.php", COMMON_API_PATHS)
    assert_in("/search.php", COMMON_API_PATHS)
    assert_in("/login.php", COMMON_API_PATHS)
    assert_in("/phpinfo.php", COMMON_API_PATHS)
    assert_gt(len(COMMON_API_PATHS), 100, "Wordlist should be comprehensive")


@test
def test_discover_flag_wired_to_auto_discover():
    """Regression: --discover flag was commented out and never reached EntropyConfig."""
    import sys, io
    from entropy.cli import cmd_run
    import argparse

    # Build minimal args namespace simulating: entropy run --discover --dry-run
    ns = argparse.Namespace(
        config=None, profile="quick", spec=None,
        target="http://localhost:19999",
        llm="mock", llm_model="", llm_api_key="", llm_url="",
        dry_run=False, live=False, concurrency=None, timeout=5.0,
        personas=None, output="/tmp/entropy-test-discover-flag",
        fail_on=None, junit=None, no_html=True,
        discover=True,          # <-- the flag being tested
        dashboard=False, dashboard_port=8080,
        watch=False, watch_interval=300, watch_file=None,
        proxy=None, no_verify_ssl=True, retries=1,
        sarif=None, no_baseline=True, no_history=True,
        history_db=None, ws=None, proxy_mode=False, proxy_port=8888,
        no_rate_limit_check=True, rate_limit_probes=5,
        diff_target=None, diff_auth_header=None,
        custom_persona=None, webhook=None, slack_webhook=None,
        github_repo=None, gitlab_project=None,
        verbose=False, no_llm_enrichment=True, env_file=".env",
    )

    from entropy.core.orchestrator import EntropyConfig
    from entropy.core.config import PROFILES, load_dotenv
    load_dotenv(ns.env_file)
    profile = PROFILES.get(ns.profile, PROFILES["standard"])
    from entropy.core.models import PersonaType
    ...
    config = EntropyConfig(
        ...
        llm_backend=ns.llm,
        dry_run=not ns.live,
        concurrency=ns.concurrency if ns.concurrency else profile.concurrency,
        timeout=ns.timeout,
        max_steps=profile.max_steps,
        personas=personas,
        output_dir=ns.output,
        fail_on=ns.fail_on or profile.fail_on,
        html_report=not ns.no_html,
        scan_profile=ns.profile,
        enrich_with_llm=not ns.no_llm_enrichment,
        auto_discover=ns.discover,  # This is what we're testing
        rate_limit_check=not ns.no_rate_limit_check,
        save_history=not ns.no_history,
        verify_ssl=not ns.no_verify_ssl,
        max_retries=ns.retries,
        baseline_diff=not ns.no_baseline,
    )
    assert_true(config.auto_discover, "auto_discover should be True when --discover flag is set")
...
# ============================================================
# Q. v0.4.0 — SSRF DETECTION
# ============================================================

@test
def test_ssrf_param_names_list():
    from entropy.fuzzing.ssrf import SSRF_PARAM_NAMES, SSRF_TARGETS
    assert_gt(len(SSRF_PARAM_NAMES), 10)
    assert_in("url", SSRF_PARAM_NAMES)
    assert_in("callback", SSRF_PARAM_NAMES)
    assert_in("redirect", SSRF_PARAM_NAMES)
    assert_gt(len(SSRF_TARGETS), 5)


@test
def test_ssrf_find_params():
    from entropy.fuzzing.ssrf import SSRFDetector
    from entropy.core.models import APIEndpoint, APIParameter, RequestMethod
    ep = APIEndpoint(
        path="/fetch", method=RequestMethod.GET,
        parameters=[
            APIParameter("url", "query", "string"),
            APIParameter("name", "query", "string"),
        ],
    )
    det = SSRFDetector("http://localhost", dry_run=True)
    params = det._find_ssrf_params(ep)
    assert_in("url", params)
    assert_true("name" not in params)


@test
def test_ssrf_mock_results():
    from entropy.fuzzing.ssrf import SSRFDetector
    from entropy.core.models import APIEndpoint, APIParameter, RequestMethod
    ep = APIEndpoint(
        path="/proxy", method=RequestMethod.GET,
        parameters=[APIParameter("url", "query", "string")],
    )
    det = SSRFDetector("http://localhost", dry_run=True)
    findings = det.test_endpoint(ep)
    # mock mode — findings list is returned (may or may not have hits)
    assert_isinstance(findings, list)


@test
def test_ssrf_to_finding():
    from entropy.fuzzing.ssrf import SSRFDetector, SSRFResult
    from entropy.core.models import Severity, FindingType
    result = SSRFResult(
        endpoint="GET /proxy", param="url",
        target="http://169.254.169.254/",
        target_label="AWS EC2 metadata",
        triggered=True,
        evidence="ami-id: ami-12345",
        severity="critical",
    )
    f = SSRFDetector._to_finding(result)
    assert_eq(f.type, FindingType.SSRF)
    assert_eq(f.severity, Severity.CRITICAL)
    assert_in("169.254.169.254", f.description)


# ============================================================
# R. v0.4.0 — SSTI DETECTION
# ============================================================

@test
def test_ssti_probes_list():
    from entropy.fuzzing.ssti import SSTI_PROBES
    assert_gt(len(SSTI_PROBES), 5)
    payloads = [p["payload"] for p in SSTI_PROBES]
    assert_in("{{7*7}}", payloads)
    assert_in("${7*7}", payloads)


@test
def test_ssti_find_string_params():
    from entropy.fuzzing.ssti import SSTIDetector
    from entropy.core.models import APIEndpoint, APIParameter, RequestMethod
    ep = APIEndpoint(
        path="/render", method=RequestMethod.POST,
        parameters=[
            APIParameter("template", "body", "string"),
            APIParameter("count",    "body", "integer"),
        ],
    )
    det = SSTIDetector("http://localhost", dry_run=True)
    params = det._find_string_params(ep)
    assert_in("template", params)
    assert_true("count" not in params)


@test
def test_ssti_confirmed_finding():
    from entropy.fuzzing.ssti import SSTIDetector, SSTIResult
    from entropy.core.models import Severity, FindingType
    result = SSTIResult(
        endpoint="POST /render", param="template",
        payload="{{7*7}}", expected="49",
        engine="Jinja2/Twig",
        response_body="Output: 49",
        confirmed=True,
    )
    f = SSTIDetector._to_finding(result)
    assert_eq(f.type, FindingType.INJECTION)
    assert_eq(f.severity, Severity.HIGH)
    assert_in("{{7*7}}", f.description)


@test
def test_ssti_rce_is_critical():
    from entropy.fuzzing.ssti import SSTIDetector, SSTIResult
    from entropy.core.models import Severity
    result = SSTIResult(
        endpoint="POST /render", param="template",
        payload="...", expected="uid=",
        engine="FreeMarker RCE",
        response_body="uid=0(root)",
        confirmed=True,
    )
    f = SSTIDetector._to_finding(result)
    assert_eq(f.severity, Severity.CRITICAL)
...
# ============================================================
# S. v0.4.0 — XXE DETECTION
# ============================================================

@test
def test_xxe_payloads_list():
    from entropy.fuzzing.xxe import XXE_PAYLOADS
    assert_gt(len(XXE_PAYLOADS), 2)
    bodies = [p["body"] for p in XXE_PAYLOADS]
    assert_true(any("SYSTEM" in b for b in bodies))
    assert_true(any("etc/passwd" in b for b in bodies))


@test
def test_xxe_skips_get_endpoints():
    from entropy.fuzzing.xxe import XXEDetector
    from entropy.core.models import APIEndpoint, RequestMethod
    ep = APIEndpoint(path="/data", method=RequestMethod.GET)
    det = XXEDetector("http://localhost", dry_run=True)
    findings = det.test_endpoint(ep)
    assert_eq(len(findings), 0, "XXE should not test GET endpoints")


@test
def test_xxe_finding_structure():
    from entropy.fuzzing.xxe import XXEDetector
    from entropy.core.models import APIEndpoint, RequestMethod, FindingType, Severity
    ep = APIEndpoint(path="/upload", method=RequestMethod.POST)
    f = XXEDetector._make_finding(
        ep,
        {"name": "Classic LFI via XXE", "content_type": "application/xml"},
        "root:x:0:0:root:/root:/bin/bash",
    )
    assert_eq(f.type, FindingType.XXE)
    assert_eq(f.severity, Severity.CRITICAL)
    assert_in("defusedxml", f.remediation)
...
@test
def test_jwt_parse_valid():
    import base64, json
    from entropy.fuzzing.jwt_tester import JWTAnalyser

    def _enc(d): return base64.urlsafe_b64encode(json.dumps(d).encode()).rstrip(b"=").decode()
    header  = _enc({"alg": "HS256", "typ": "JWT"})
    payload = _enc({"sub": "1234567890", "role": "user", "iat": 1516239022})
    sig     = base64.urlsafe_b64encode(b"fakesig").rstrip(b"=").decode()
    token   = f"{header}.{payload}.{sig}"

    jwt = JWTAnalyser._parse(token)
    assert_true(jwt is not None)
    assert_eq(jwt.header["alg"], "HS256")
    assert_eq(jwt.payload["sub"], "1234567890")


@test
def test_jwt_none_token_construction():
    import base64, json
    from entropy.fuzzing.jwt_tester import JWTAnalyser

    def _enc(d): return base64.urlsafe_b64encode(json.dumps(d).encode()).rstrip(b"=").decode()
    header  = _enc({"alg": "HS256", "typ": "JWT"})
    payload = _enc({"sub": "user", "role": "user"})
    token   = f"{header}.{payload}.sig"

    analyser  = JWTAnalyser()
    jwt       = analyser._parse(token)
    none_token = analyser._build_none_token(jwt)

    # Decode the none token
    parts = none_token.split(".")
    h = json.loads(base64.urlsafe_b64decode(parts[0] + "=="))
    assert_eq(h["alg"], "none")
    assert_eq(parts[2], "")  # no signature


@test
def test_jwt_weak_secret_crack():
    import base64, hashlib, hmac as hmac_mod, json
    from entropy.fuzzing.jwt_tester import JWTAnalyser

    secret = "secret"
    def _enc(d): return base64.urlsafe_b64encode(json.dumps(d).encode()).rstrip(b"=").decode()

    header_str  = _enc({"alg": "HS256", "typ": "JWT"})
    payload_str = _enc({"sub": "test"})
    message     = f"{header_str}.{payload_str}".encode()
    sig         = hmac_mod.new(secret.encode(), message, hashlib.sha256).digest()
    sig_enc     = base64.urlsafe_b64encode(sig).rstrip(b"=").decode()
    token       = f"{header_str}.{payload_str}.{sig_enc}"

    analyser = JWTAnalyser()
    jwt = analyser._parse(token)
    cracked = analyser._crack_secret(jwt)
    assert_eq(cracked, "secret")


@test
def test_jwt_no_exp_claim_flagged():
    import base64, json
    from entropy.fuzzing.jwt_tester import JWTAnalyser

    def _enc(d): return base64.urlsafe_b64encode(json.dumps(d).encode()).rstrip(b"=").decode()
    header  = _enc({"alg": "HS256"})
    payload = _enc({"sub": "user", "role": "admin"})  # no exp
    token   = f"{header}.{payload}.sig"

    analyser = JWTAnalyser()
    jwt = analyser._parse(token)
    finding = analyser._inspect_claims(jwt)
    assert_true(finding is not None)
    assert_true("exp" in finding.description.lower())


@test
def test_jwt_extract_from_response():
    import base64, json
    from entropy.fuzzing.jwt_tester import extract_jwts
    ...
    data = {"message": "login ok", "token": token}
    tokens = extract_jwts(data)
    assert_in(token, tokens)


# ============================================================
# U. v0.4.0 — PARAMETER MINING
# ============================================================

@test
def test_param_miner_priority_list():
    from entropy.fuzzing.param_miner import PRIORITY_PARAMS
    assert_gt(len(PRIORITY_PARAMS), 30)
    assert_in("admin", PRIORITY_PARAMS)
    assert_in("debug", PRIORITY_PARAMS)
    assert_in("is_admin", PRIORITY_PARAMS)
    assert_in("role", PRIORITY_PARAMS)


@test
def test_param_miner_severity_critical():
    from entropy.fuzzing.param_miner import ParameterMiner
    assert_eq(ParameterMiner._assess_severity("is_admin"), "critical")
    assert_eq(ParameterMiner._assess_severity("bypass"), "critical")
    assert_eq(ParameterMiner._assess_severity("debug"), "high")
    assert_eq(ParameterMiner._assess_severity("limit"), "medium")


@test
def test_param_miner_is_different_status():
    from entropy.fuzzing.param_miner import ParameterMiner
    baseline = (200, '{"id":1}', {"id": 1})
    probe    = (403, '',          {})
    assert_true(ParameterMiner._is_different(baseline, probe))


@test
def test_param_miner_is_different_new_keys():
    from entropy.fuzzing.param_miner import ParameterMiner
    baseline = (200, '{"id":1}',              {"id": 1})
    probe    = (200, '{"id":1,"admin":true}', {"id": 1, "admin": True})
    assert_true(ParameterMiner._is_different(baseline, probe))


@test
def test_param_miner_identical_not_different():
    from entropy.fuzzing.param_miner import ParameterMiner
    baseline = (200, '{"ok":true}', {"ok": True})
    probe    = (200, '{"ok":true}', {"ok": True})
    assert_true(not ParameterMiner._is_different(baseline, probe))


@test
def test_param_miner_finding_structure():
    from entropy.fuzzing.param_miner import ParameterMiner, ParamHit
    from entropy.core.models import Severity
    hit = ParamHit(
        endpoint="GET /api/users",
        param="admin",
        location="query",
        evidence="Status changed: 200 → 403",
        severity="critical",
    )
    f = ParameterMiner._to_finding(hit)
    assert_eq(f.severity, Severity.CRITICAL)
    assert_in("admin", f.title)


# ============================================================
# V. v0.4.0 — IDOR CHAIN TESTER
# ============================================================

@test
def test_idor_chain_find_resource_endpoints():
    from entropy.fuzzing.idor_chain import IDORChainTester
    from entropy.core.models import APISchema, APIEndpoint, APIParameter, RequestMethod

    schema = APISchema(
        title="Test", version="1", base_url="http://localhost",
        endpoints=[
            APIEndpoint(path="/users/{user_id}", method=RequestMethod.GET,
                        parameters=[APIParameter("user_id", "path", "integer")]),
            APIEndpoint(path="/search", method=RequestMethod.GET),
            APIEndpoint(path="/orders/{order_id}", method=RequestMethod.GET,
                        parameters=[APIParameter("order_id", "path", "integer")]),
        ]
    )
    tester = IDORChainTester("http://localhost", dry_run=True)
    eps    = tester._find_resource_endpoints(schema)
    paths  = [e.path for e in eps]
    assert_in("/users/{user_id}", paths)
    assert_in("/orders/{order_id}", paths)
    assert_true("/search" not in paths)


@test
def test_idor_chain_finding():
    from entropy.fuzzing.idor_chain import IDORChainTester, IDORChainResult
    from entropy.core.models import Severity, FindingType
    result = IDORChainResult(
        endpoint_a="GET /users/{id}", endpoint_b="GET /users/2",
        resource_id=2, user_a_id=1, user_b_id=2,
        response_body='{"email":"victim@test.com"}',
        confirmed=True,
    )
    f = IDORChainTester._to_finding(result)
    assert_eq(f.type, FindingType.IDOR)
    assert_eq(f.severity, Severity.HIGH)
    assert_in("2", f.title)


@test
def test_idor_chain_contains_sensitive():
    from entropy.fuzzing.idor_chain import IDORChainTester
    tester = IDORChainTester("http://localhost", dry_run=True)
    assert_true(tester._contains_sensitive_data({"email": "x@y.com", "balance": 100}))
    assert_true(not tester._contains_sensitive_data({"status": "ok"}))


# ============================================================
# W. v0.4.0 — ADAPTIVE ANALYSER
# ============================================================

@test
def test_adaptive_analyser_confirms_real_finding():
    from entropy.fuzzing.adaptive_analyser import AdaptiveResponseAnalyser
    from entropy.llm.backends import MockLLM
    from entropy.core.models import Finding, FindingType, Severity, TestStep, HTTPRequest, HTTPResponse

    analyser = AdaptiveResponseAnalyser(MockLLM(), min_confidence=0.5)
    step = TestStep(
        step_number=1, description="test",
        request=HTTPRequest("GET", "http://t/search", body={"q": "sql syntax"}),
        response=HTTPResponse(500, body={"error": "You have an error in your SQL syntax"}),
    )
    f = Finding(
        type=FindingType.INJECTION, severity=Severity.HIGH,
        title="SQLi", endpoint="GET /search", steps=[step],
    )
    confirmed, rejected = analyser.filter_false_positives([f])
    assert_gt(len(confirmed), 0)


@test
def test_adaptive_analyser_rejects_false_positive():
    from entropy.fuzzing.adaptive_analyser import AdaptiveResponseAnalyser
    from entropy.llm.backends import MockLLM
    from entropy.core.models import Finding, FindingType, Severity, TestStep, HTTPRequest, HTTPResponse
    ...
    step = TestStep(
        step_number=1, description="test",
        request=HTTPRequest("GET", "http://t/api", body={"q": "test"}),
        response=HTTPResponse(400, body={"error": "Bad Request"}),
    )
    f = Finding(
        type=FindingType.INJECTION, severity=Severity.HIGH,
        title="False Positive Check",
        description="400 bad request response",
        endpoint="GET /api", steps=[step],
    )
    confirmed, rejected = analyser.filter_false_positives([f])
    # With mock LLM, bad-request scenario should be rejected
    total = len(confirmed) + len(rejected)
    assert_eq(total, 1)


@test
def test_adaptive_analyser_passthrough_no_steps():
    from entropy.fuzzing.adaptive_analyser import AdaptiveResponseAnalyser
    from entropy.llm.backends import MockLLM
    from entropy.core.models import Finding, FindingType, Severity

    analyser = AdaptiveResponseAnalyser(MockLLM())
    f = Finding(
        type=FindingType.DATA_LEAK, severity=Severity.MEDIUM,
        title="No Steps", endpoint="GET /data",
    )
    confirmed, rejected = analyser.filter_false_positives([f])
    assert_in(f, confirmed, "Findings without step data should pass through")


# ============================================================
# X. v0.4.0 — HTTP SMUGGLING
# ============================================================

@test
def test_smuggling_probes_not_empty():
    from entropy.fuzzing.smuggling import CL_TE_PROBE, TE_CL_PROBE
    assert_in("Transfer-Encoding", CL_TE_PROBE)
    assert_in("Content-Length", CL_TE_PROBE)
    assert_in("Transfer-Encoding", TE_CL_PROBE)


@test
def test_smuggling_dry_run_returns_empty():
    from entropy.fuzzing.smuggling import RequestSmugglingDetector
    det = RequestSmugglingDetector("http://localhost:9999", dry_run=True)
    findings = det.detect()
    assert_eq(findings, [], "Dry-run mode should return no findings")


@test
def test_smuggling_finding_structure():
    from entropy.fuzzing.smuggling import RequestSmugglingDetector, SmuggleResult
    from entropy.core.models import Severity, FindingType
    result = SmuggleResult("CL.TE", "test.host", "/api", 5200.0, True)
    f = RequestSmugglingDetector._to_finding(result)
    assert_eq(f.type, FindingType.LOGIC_ERROR)
    assert_eq(f.severity, Severity.CRITICAL)
    assert_in("CL.TE", f.title)


# ============================================================
# Y. v0.4.0 — NEW FINDING TYPES IN MODELS
# ============================================================

@test
def test_new_finding_types_exist():
    from entropy.core.models import FindingType
    assert_true(hasattr(FindingType, "SSRF"))
    assert_true(hasattr(FindingType, "XXE"))
    assert_true(hasattr(FindingType, "SSTI"))
    assert_true(hasattr(FindingType, "SMUGGLING"))
    assert_true(hasattr(FindingType, "PARAMETER_POLLUTION"))
    assert_true(hasattr(FindingType, "IDOR"))


@test
def test_sarif_owasp_new_types():
    from entropy.reporting.sarif import _OWASP_TAGS
    assert_in("xxe", _OWASP_TAGS)
    assert_in("ssti", _OWASP_TAGS)
    assert_in("smuggling", _OWASP_TAGS)
    assert_eq(_OWASP_TAGS["xxe"], ["A05:2021"])


# ============================================================
# Z. v0.4.0 — FULL INTEGRATION (all new modules)
# ============================================================

@test
def test_v040_full_pipeline_dry_run():
    """Full v0.4.0 dry-run: all new modules active, 0 crashes."""
    import tempfile
    from pathlib import Path
    from entropy.core.orchestrator import EntropyConfig, EntropyRunner
    from entropy.core.models import PersonaType, TestStatus
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        config = EntropyConfig(
            ...
            dry_run      = True,
            output_dir   = str(Path(tmpdir) / "reports"),
            personas     = [PersonaType.PENETRATION_TESTER, PersonaType.MALICIOUS_INSIDER],
            scan_profile = "full",
            html_report  = True,
            cvss_scoring = True,
            enrich_with_llm     = True,
        ...
        assert_eq(report.status, TestStatus.COMPLETED)
        assert_gt(report.stats["requests_sent"], 0)
        # SSRF/SSTI/XXE modules ran (even if no findings in dry-run)
        assert_true(Path(tmpdir, "reports", "report.json").exists())
        assert_true(Path(tmpdir, "reports", "report.html").exists())


@test
def test_v040_version():
    from entropy import __version__
    assert_eq(__version__, "0.4.4")


@test
def test_v040_config_new_flags_default():
    from entropy.core.orchestrator import EntropyConfig
    cfg = EntropyConfig()
    assert_true(cfg.ssrf_check)
    assert_true(cfg.ssti_check)
    assert_true(cfg.xxe_check)
    assert_true(cfg.jwt_check)
    assert_true(cfg.param_mining)
    assert_true(cfg.idor_chain)
    assert_true(cfg.adaptive_analysis)
    assert_true(not cfg.smuggling_check)   # off by default


@test
def test_v040_mock_llm_adaptive_verdict():
    from entropy.llm.backends import MockLLM
    llm = MockLLM()
    result = llm.complete_json("Review this potential finding: genuine security issue with sql syntax error in response. confirmed?")
    assert_in("confirmed", result)
    assert_in("confidence", result)
    assert_in("reasoning", result)


# ============================================================
# Z2. v0.4.4 — SMART CRAWLER
# ============================================================

@test
def test_crawler_same_domain_filter():
    from entropy.discovery.crawler import ActiveCrawler
    c = ActiveCrawler("http://example.com")
    assert_true(c._is_same_domain("http://example.com/page"))
    assert_true(c._is_same_domain("http://sub.example.com/api"))
    assert_true(not c._is_same_domain("http://evil.com/page"))
    assert_true(not c._is_same_domain("https://google.com"))


@test
def test_crawler_normalise_url():
    from entropy.discovery.crawler import ActiveCrawler
    c = ActiveCrawler("http://example.com")
    assert_eq(c._normalise_url("http://example.com/page"), "http://example.com/page")
    assert_eq(c._normalise_url("/api/v1"), "http://example.com/api/v1")
    assert_eq(c._normalise_url("http://example.com/img.png"), None)
    assert_eq(c._normalise_url("http://example.com/style.css"), None)
    assert_eq(c._normalise_url(""), None)
    # Fragment stripped
    result = c._normalise_url("http://example.com/page#section")
    assert_eq(result, "http://example.com/page")


@test
def test_crawler_extract_links_same_domain_only():
    from entropy.discovery.crawler import ActiveCrawler
    c = ActiveCrawler("http://testphp.vulnweb.com")
    html = """
    <a href="/login.php">Login</a>
    <a href="http://testphp.vulnweb.com/cart.php">Cart</a>
    <a href="https://evil.com/steal">Evil</a>
    <a href="http://google.com/search">Google</a>
    <a href="/api/v1">API</a>
    """
    links = c._extract_links(html, "http://testphp.vulnweb.com/")
    # Only same-domain links
    assert_true(all("testphp.vulnweb.com" in l for l in links))
    assert_true(not any("evil.com" in l for l in links))
    assert_true(not any("google.com" in l for l in links))


@test
def test_crawler_parse_forms():
    from entropy.discovery.crawler import ActiveCrawler
    c = ActiveCrawler("http://testphp.vulnweb.com")
    html = """
    ...
    """
    forms = c._parse_forms(html, "http://testphp.vulnweb.com/")
    assert_eq(len(forms), 2)
    post_form = next(f for f in forms if f.method == "POST")
    assert_true("login.php" in post_form.action)
    names = [i.name for i in post_form.inputs]
    assert_in("username", names)
    assert_in("password", names)
    assert_true(post_form.has_csrf_token)


@test
def test_form_spec_injectable_inputs():
    from entropy.discovery.crawler import FormSpec, FormInput
    form = FormSpec(action="/submit", method="POST", inputs=[
        FormInput("username", "text"),
        FormInput("password", "password"),
        FormInput("token", "hidden", "abc"),
        FormInput("submit_btn", "submit"),
        FormInput("file", "file"),
    ])
    injectable = form.injectable_inputs
    names = [i.name for i in injectable]
    assert_in("username", names)
    assert_in("password", names)
    # submit and file should be excluded
    assert_true("submit_btn" not in names)
    assert_true("file" not in names)


@test
def test_form_spec_csrf_detection():
    from entropy.discovery.crawler import FormSpec, FormInput
    form_no_csrf = FormSpec(action="/login", method="POST", inputs=[
        FormInput("username", "text"), FormInput("password", "password"),
    ])
    assert_true(not form_no_csrf.has_csrf_token)

    form_with_csrf = FormSpec(action="/login", method="POST", inputs=[
        FormInput("username", "text"),
        FormInput("csrfmiddlewaretoken", "hidden", "xyz"),
    ])
    assert_true(form_with_csrf.has_csrf_token)


@test
def test_form_fuzzer_csrf_check():
    from entropy.discovery.crawler import FormSpec, FormInput
    from entropy.fuzzing.form_fuzzer import FormFuzzer
    from entropy.core.models import FindingType

    ff = FormFuzzer()
    # POST form with no CSRF token and meaningful input
    form = FormSpec(action="http://test.local/login", method="POST", inputs=[
        FormInput("username", "text"),
        FormInput("password", "password"),
    ])
    finding = ff._check_csrf(form)
    assert_true(finding is not None)
    assert_eq(finding.type, FindingType.CSRF)

    # POST form WITH csrf token → no finding
    form_safe = FormSpec(action="http://test.local/login", method="POST", inputs=[
        FormInput("username", "text"),
        FormInput("csrf_token", "hidden", "xyz"),
    ])
    finding2 = ff._check_csrf(form_safe)
    assert_eq(finding2, None)

    # GET form → no CSRF finding needed
    form_get = FormSpec(action="http://test.local/search", method="GET", inputs=[
        FormInput("q", "text"),
    ])
    finding3 = ff._check_csrf(form_get)
    assert_eq(finding3, None)


@test
def test_url_injector_param_map_from_urls():
    from entropy.fuzzing.url_injector import URLParamInjector
    inj = URLParamInjector("http://testphp.vulnweb.com")
    urls = [
        "http://testphp.vulnweb.com/listproducts.php?cat=1",
        "http://testphp.vulnweb.com/artists.php?artist=2",
        "http://testphp.vulnweb.com/product.php?pic=1&id=2",
    ]
    pmap = inj.build_param_map_from_urls(urls)
    assert_in("/listproducts.php", pmap)
    assert_in("cat", pmap["/listproducts.php"])
    assert_in("/artists.php", pmap)
    assert_in("artist", pmap["/artists.php"])
    assert_in("/product.php", pmap)
    assert_in("pic", pmap["/product.php"])
    assert_in("id", pmap["/product.php"])


@test
def test_v044_version():
    from entropy import __version__
    assert_eq(__version__, "0.4.4")


@test
def test_v044_config_new_flags_default():
    from entropy.core.orchestrator import EntropyConfig
    cfg = EntropyConfig()
    assert_true(cfg.form_fuzz)
    assert_true(cfg.url_inject)
    assert_eq(cfg.crawl_max_depth, 4)
    assert_eq(cfg.crawl_max_pages, 80)


@test
def test_csrf_finding_type_exists():
    from entropy.core.models import FindingType
    assert_true(hasattr(FindingType, "CSRF"))
    assert_eq(FindingType.CSRF.value, "csrf")


@test
def test_sarif_csrf_owasp_tag():
    from entropy.reporting.sarif import _OWASP_TAGS
    assert_in("csrf", _OWASP_TAGS)
    assert_in("A01:2021", _OWASP_TAGS["csrf"])
